...
func _physics_process(delta):
    if keydown_mouseleft and mouse_is_over:
        mousepoint = self.get_global_mouse_position()
        mouse_distance = self.get_position().distance_to(mousepoint)
        is_dragging = true
        ...
        if (mouse_distance <= 5.0):
            ...
            rigidbody_vector = (mousepoint - self.get_position()).normalized()
